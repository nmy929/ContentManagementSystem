...
echo "[3/6] Loading schema and data if needed"
HAS_ARTICLES=$(docker compose exec -T db psql -U d551user -d d551 -tAc "SELECT to_regclass('public.articles');")
if [[ "$HAS_ARTICLES" != "articles" ]]; then
...
curl -s -X POST http://localhost:8000/api/admin/run_load_test \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"target":"article_views","concurrency":200,"ops":10000}' >/dev/null
